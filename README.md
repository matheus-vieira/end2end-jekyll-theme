...
The theme can gate GA4 behind a lightweight consent banner without adding any external UI library.

- **Enable or disable:** set `cookie_consent: true` or `cookie_consent: false` in `_config.yml`.
- **Storage:** the banner stores the user choice in `localStorage` under `cookie_consent_storage_key`.
- **Copy overrides:** customize the banner text with `cookie_consent_title`, `cookie_consent_message`, `cookie_consent_accept_label`, and `cookie_consent_decline_label` in `_config.yml`, or override the defaults in `source/_data/cookie_consent.yml`.
- **Analytics:** GA4 still requires `ga_measurement_id`; the script is injected only after an explicit accept.

If you disable the banner, the analytics include falls back to the existing production-only behavior.
